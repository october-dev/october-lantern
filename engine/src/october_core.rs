...
pub struct Client {
    ...
    agent: ureq::Agent,
    next: u64,
    /// Canvases whose last snapshot had running agents: read once more after their terminals
    /// stop, so those agents drop off.
    busy: Vec<String>,
...
impl Client {
    ...
    pub fn new(run: RunFile, paired: Option<(String, String)>) -> Client {
        ...
        Client { run, principal, agent, next: 0, busy: Vec::new(), last_reads: 0 }
    }
    ...
    pub fn agents(&mut self) -> Result<Vec<OctoberAgent>> {
        let mut out = Vec::new();
        if self.paired() {
            // Only canvases with something running: one `terminal.list` says which (every
            // running terminal carries its canvas), then each of those canvases' snapshot. Idle
            // canvases, however many, cost nothing.
            let terminals = self.call("terminal.list", json!({}), false)?;
            let mut canvases: Vec<String> = terminals
                .as_array()
                .into_iter()
                .flatten()
                .filter(|t| !matches!(t["state"].as_str(), Some("stopped" | "failed" | "exited" | "dead")))
                .filter_map(|t| t["canvasId"].as_str().map(String::from))
                .collect();
            canvases.extend(self.busy.iter().cloned());
            canvases.sort();
            canvases.dedup();
            self.last_reads = canvases.len() + 1;
            self.busy.clear();
            for canvas in &canvases {
                let agents = self.canvas_agents(canvas)?;
                if !agents.is_empty() {
                    self.busy.push(canvas.clone());
                }
                out.extend(agents);
            }
        } else {
