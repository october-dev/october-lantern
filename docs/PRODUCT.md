...
- **Point & Ask:** hold ⌃⌥P (Settings › General can change it), point at something on screen, say what you want and let go. A card by the pointer shows a screenshot of about 900×600 points around the pointer (Lantern's own windows left out, the pointer marked with a ring), the app and window, the page's address in Safari, Chrome, Arc, Brave or Edge, and any selected text, plus the agent it goes to: the one working on the open document's project, the one named in the window, the one in the terminal in front, or the agent you last messaged. Enter sends, Esc cancels; a quick tap instead of a hold keeps listening until you press the shortcut again. The screenshot is saved in the agent's project under `.lantern/shots/`, which Lantern adds to the repository's local exclude list (`.git/info/exclude`, never `.gitignore`), so it isn't committed; the agent gets its path with your words.
...
- **Point & Ask** captures only while you hold its shortcut: a screenshot around the pointer, the front app's name and window title, the browser's page address (macOS asks once per browser) and the selected text (with Accessibility). It's saved in the agent's project (`.lantern/shots/`, excluded from git) and given to that agent; Lantern sends it nowhere else.
